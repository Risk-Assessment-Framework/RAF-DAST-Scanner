...
class OWASPScanner:
    ...
    def active_scan(self):
        activeScanId = self.zap.ascan.scan(self.target)
        while int(self.zap.ascan.status(activeScanId)) < 100:
            print('Active Scan progress %: {}'.format(
                self.zap.ascan.status(activeScanId)))
            time.sleep(3)
